...
{
protected:
	std::vector<mca::ComposedTag> tagList;
	std::string filePath;
	std::string fileKey;
	int codekey;
protected:
...
public:
	...
	mca::ComposedTag& getTag(std::string const&key)
	{
		for(mca::ComposedTag&tag:tagList)
			if(tag.getKey()==key)
				return tag;
		mca::ComposedTag newTag(key);
		return newTag;
	}
	void addTag(mca::ComposedTag tag)
	{
		if(!hasTag(tag.getKey()))
			tagList.push_back(tag);
	}
	void removeTag(std::string const&key)
	{
		std::vector<mca::ComposedTag>::iterator it;
